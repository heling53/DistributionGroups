...
.DESCRIPTION
    «Все сотрудники» — вершина дерева рассылки, которой НЕТ в выгрузке 1С
    (Подразделения.xlsx). Она создаётся вручную ОДИН РАЗ этим скриптом, после чего на неё
    опирается Sync-DistributionGroups.ps1 (параметр $RootGroupName):
      - как подразделение группа не управляется совсем: ни создания, ни переименования,
        ни карантина, ни записей в файлы состояния — она отсеивается сразу после каждого
        чтения групп из AD, поэтому шаг 4 её не видит и удалить не может;
      - ведётся только вложенность: шаг 2.1 складывает внутрь «Руководство»
        ($NoChildGroups) и отделы, у которых в выгрузке вышестоящим подразделением
        указано «Руководство». Посторонние члены вершины не трогаются.

    Группе намеренно присваивается то же Description, что и у остальных групп рассылки
    ('Группа рассылки подразделения_v1.0'), чтобы её находил аудит и чтобы синхронизация
    видела её среди своих групп — отсев вершины делается по имени, а не по описанию.
